...
// Стили для продакшена без минификации
const stylesProd = () => {
  return (
    src(paths.srcScss)
      .pipe(
      ...
      // Удалены gulpif и cleanCSS, которые отвечали за минификацию
      .pipe(dest(paths.buildCssFolder))
      .pipe(browserSync.stream())
  );
};

// Скрипты для продакшена без минификации
const scriptsProd = () => {
  return src(paths.srcMainJs)
    .pipe(
      plumber(
...
// Эта задача будет вызвана, когда вы захотите собрать ваши файлы для продакшна без минификации
exports.buildProdWithoutMinification = series(
  clean,
  htmlInclude,
  scriptsProd,
  stylesProd,
